        ...
        >
            ...
            <div className="flex justify-between items-end">
                ...
                <div className="flex flex-col items-end gap-2 pl-2">
                    <p className="text-sm font-bold text-muted-foreground font-mono mb-1">{formatDate(venda.data)}</p>
                    <div className="flex items-center gap-1">
                        ...
                        {venda.status === 'cancelada' && (
                            <Button
                                size="icon"
                                variant="ghost"
                                className="h-8 w-8 text-destructive hover:text-destructive/80 hover:bg-destructive/10 rounded-lg"
...
export function VendasHistory({ contatoId }: { contatoId: string }) {
    ...
    if (loading) return <div className="text-center py-8 text-muted-foreground text-sm">Carregando histórico...</div>
    if (error) return <div className="text-center py-8 text-destructive text-sm">Erro ao carregar histórico</div>
    if (vendas.length === 0) return <div className="text-center py-8 text-muted-foreground text-sm opacity-60">Nenhuma compra registrada 🍃</div>
            ...
            <div className="mt-2">
                ...
                >
                    <div className="space-y-4">
                        <p className="text-muted-foreground">Confirma exclusão desta venda cancelada?</p>
                        <ModalActions>
