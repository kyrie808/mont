...
export function VendaReceipt({ venda }: VendaReceiptProps) {
    // Brilho neon (modo tactical/escuro) — decorativo, referencia o RGB de --primary
    const neonShadow = "dark:drop-shadow-[0_0_10px_rgba(19,236,19,0.5)]"

    return (
        <div className="relative w-full mb-8">
            <div className="bg-card pt-8 px-6 pb-6 rounded-t-xl relative border-x border-t border-border shadow-xl dark:shadow-2xl">
                <div className="flex flex-col items-center mb-8">
                    <span className="text-xs font-mono text-muted-foreground dark:text-primary/60 tracking-[0.2em] mb-2 uppercase">Valor Total</span>
                    <h1 className={`text-4xl font-mono font-bold text-foreground dark:text-primary tracking-tight mb-6 ${neonShadow}`}>
                ...
                <div className="flex flex-col space-y-4">
                    {venda.itens.map((item) => (
                        <div key={item.id} className="flex justify-between items-start group">
                            ...
                            <span className="text-sm font-mono text-foreground group-hover:text-primary transition-colors tabular-nums">
                                {formatCurrency(item.subtotal)}
                            </span>
                        </div>
                    ))}

                    <div className="border-b border-dashed border-border w-full my-4"></div>
                    ...
                    {venda.pago ? (
                        <div className="flex justify-center mt-4 pt-3 border-t border-dashed border-border">
                            <span className="text-xs font-bold text-success uppercase tracking-wider">✓ Quitado</span>
                        </div>
                    ) : venda.pagamentos.length > 0 && (
                        <div className="mt-4 pt-3 border-t border-dashed border-border">
                            <span className="text-[10px] font-mono text-muted-foreground uppercase tracking-wider block mb-2">Pagamentos recebidos</span>
