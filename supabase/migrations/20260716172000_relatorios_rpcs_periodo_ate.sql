-- Relatórios: período vira Mês OU Geral (o diretor removeu os lookbacks 30d/90d/6m/1a).
--
-- As 5 RPCs ganham `p_ate` (fim do intervalo) além de `p_desde`:
--   Geral = ambos NULL;  Mês = p_desde=1º dia, p_ate=último dia.
-- Assinatura nova (2 params) seria um OVERLOAD da de 1 param → DROP antes de recriar.
-- Corpo idêntico ao de 20260716165000, só somando `AND (p_ate IS NULL OR data <= p_ate)`.

DROP FUNCTION IF EXISTS public.rpt_ltv_por_cliente_periodo(date);
DROP FUNCTION IF EXISTS public.rpt_margem_por_sku_periodo(date);
DROP FUNCTION IF EXISTS public.rpt_giro_estoque_periodo(date);
DROP FUNCTION IF EXISTS public.rpt_distribuicao_forma_pagamento_periodo(date);
DROP FUNCTION IF EXISTS public.rpt_marketing_pedidos_periodo(date);

-- 1) LTV por cliente ------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.rpt_ltv_por_cliente_periodo(p_desde date DEFAULT NULL, p_ate date DEFAULT NULL)
RETURNS TABLE (
...
-- 2) Margem por SKU -------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.rpt_margem_por_sku_periodo(p_desde date DEFAULT NULL, p_ate date DEFAULT NULL)
RETURNS TABLE (
...
CREATE OR REPLACE FUNCTION public.rpt_distribuicao_forma_pagamento_periodo(p_desde date DEFAULT NULL, p_ate date DEFAULT NULL)
RETURNS TABLE (
...
CREATE OR REPLACE FUNCTION public.rpt_marketing_pedidos_periodo(p_desde date DEFAULT NULL, p_ate date DEFAULT NULL)
RETURNS TABLE (
...
AS $function$
    ...
    WHERE (p_desde IS NULL OR data_venda >= p_desde)
      AND (p_ate IS NULL OR data_venda <= p_ate)
    GROUP BY data_venda
